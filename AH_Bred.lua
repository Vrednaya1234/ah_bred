...
function explode_argb(argb)
	...
	if i_setting_items.v then
		...
		imgui.Text(fa.ICON_VIDEO_CAMERA ..u8" Кастомное наблюдение за игроком.")
		imgui.SameLine()
		imgui.SetCursorPosX(imgui.GetWindowWidth() - 35)
		imgui.ToggleButton("##1", setting_items.hide_td)
		imgui.Text(fa.ICON_COMMENT .. u8" Быстрые ответы на ANS.")
		imgui.SameLine()
		imgui.SetCursorPosX(imgui.GetWindowWidth() - 35)
		...
		imgui.Text(fa.ICON_TABLE .. u8" Кастомный ScoreBoard (TAB).")
		imgui.SameLine()
		imgui.SetCursorPosX(imgui.GetWindowWidth() - 35)
		imgui.ToggleButton("##4", setting_items.Custom_SB)
		
		imgui.Separator()
		imgui.Text(fa.ICON_MICROPHONE_SLASH .. u8" Автомут за мат")
		imgui.SameLine()
		imgui.SetCursorPosX(imgui.GetWindowWidth() - 35)
		imgui.ToggleButton("##5", setting_items.Chat_Logger)
		
		imgui.Text(fa.ICON_MICROPHONE_SLASH .. u8" Автомут за оск/унижение")
		imgui.SameLine()
		imgui.SetCursorPosX(imgui.GetWindowWidth() - 35)
		...
		imgui.Separator()
		imgui.Text(fa.ICON_TABLE .. u8" Сокращенные команды наказаний.")
		imgui.SameLine()
		imgui.SetCursorPosX(imgui.GetWindowWidth() - 35)
		imgui.ToggleButton("##7", setting_items.Punishments)
		
		imgui.Text(fa.ICON_WHATSAPP .. u8" Админ чат.")
		imgui.SameLine()
		imgui.SetCursorPosX(imgui.GetWindowWidth() - 35)
		imgui.ToggleButton("##8", setting_items.Admin_chat)
		imgui.Text(fa.ICON_WHATSAPP .. u8" Прозрачность админ чата.")
		imgui.SameLine()
		imgui.SetCursorPosX(imgui.GetWindowWidth() - 35)
		imgui.ToggleButton("##9", setting_items.Transparency)
		imgui.Text(fa.ICON_TIMES .. u8" Закрытие начальных диалогов")
		imgui.SameLine()
		imgui.SetCursorPosX(imgui.GetWindowWidth() - 35)
		...
		imgui.Text(fa.ICON_FLAG .. u8" Автоловля репорта")
		imgui.SameLine()
		imgui.SetCursorPosX(imgui.GetWindowWidth() - 35)
		...
		imgui.Separator()
		if imgui.CollapsingHeader(fa.ICON_COG .. u8" Настройки") then
		imgui.InputText(u8"Приветствие.", HelloAC)
		imgui.InputText(fa.ICON_KEY .. u8" Админ пароль", AdminPassword)
		imgui.PushItemWidth(190)
		if imgui.Combo(fa.ICON_LEVEL_DOWN .. u8' Админ уровень', admlvl, arr_admlvl) then
		if admlvl.v == 0 then
		...
		if setting_items.Admin_chat.v then
			if imgui.Button(fa.ICON_COG .. u8' Настройка админ чата.', btn_size) then
				i_admin_chat_setting.v = not i_admin_chat_setting.v
			end
		end
		if imgui.Button(fa.ICON_COG .. u8" Настройка клавиш скрипта.") then
			setting_keys = true
		end
		
		end
		if admlvl.v == 17 then
		if imgui.CollapsingHeader(fa.ICON_COG .. u8' Настройки префиксов') then
		...
		if imgui.Button(fa.ICON_FLOPPY_O .. u8" Сохранить.") then
		...
		if imgui.Button(fa.ICON_POWER_OFF .. u8" Отключить.") then
			lua_thread.create(function ()
				imgui.Process = false
				wait(200)
		...
		if imgui.Button(fa.ICON_REFRESH ..u8" Перезагрузить.") then
			imgui.ShowCursor = false
		...
		if setting_keys then
			...
			imgui.Begin(fa.ICON_COG .. u8" Настройка клавиш.")
